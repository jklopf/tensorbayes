...
Y = tf.constant(y, shape=[N,1], dtype=tf.float32)
beta_true = np.loadtxt('beta_true.csv', delimiter=',').reshape([M,1])
record_defaults = [tf.float32] * N # N required columns
...
data_x = tf.data.experimental.CsvDataset('x.csv', record_defaults) # reflects the randomly selected column
