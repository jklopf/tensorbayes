...
# TODO: implement dataset API here
# 1. transpose the numpy array
# 2. dataset from slice 
# 3. shuffle and repeat

Y = tf.constant(y, dtype=tf.float32, shape=[N,1])


# CAREFUL : THE DATASET SHOULD RETAIN ITS INTEGRITY
# to test if we keep the same order of ten rows (column)
...
# TODO: is a variable collection necessary ? what could be the advantages to have a collection ?

## VARIABLES (if the value changes, it is a variable)
...
betaj = tf.placeholder(tf.float32, shape=())
betas = tf.placeholder(tf.float32, shape=(M,1))
ind_ = tf.placeholder(tf.int32, shape=())


# concatenations maybe ?????
